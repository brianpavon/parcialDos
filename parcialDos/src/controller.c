 ...
 *
 */
int controller_loadFromText(char* path , LinkedList* pAarrayPuppyList)
{
    int retorno = -1;
	FILE* pFile;
	pFile = fopen(path, "r");
	...
	if(pFile != NULL)
	{
		retorno = parser_cachorrosParser(pFile,pAarrayPuppyList);
	}
	fclose(pFile);
 ...
 *
 */
/*int controller_saveAsText(char* path , LinkedList* pAarrayPuppyList)
{
    int retorno = -1;
    int i;
    FILE* pFile;
